...
class RowBatchListenerTest {
    ...
    fun `table clone and events`() {
        ...
        val t2 = batch(t1) {
            for (c in listOf("A", "B", "C", "D")) {
                for (r in 1..100) {
                    t1[c][r] = "$c$r A1"
                    // these will be overwritten if (r == 1) expectedT1EventCount++
                }
            }
    ...
    fun `type filtering for subscriptions`() {
        ...
        batch(t1) { t1["B", 0] = 200 } // Auto converted to Long
