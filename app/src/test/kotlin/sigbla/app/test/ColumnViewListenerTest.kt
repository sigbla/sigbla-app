...
class ColumnViewListenerTest {
    ...
    fun `subscribe and unsubscribe cellview`() {
        ...
        tv1["A", 4][CellHeight] = 175
        tv1["A", 4][CellWidth] = 175
    ...
    fun `listener loop support`() {
        ...
        val ref1 = on(t["A"], skipHistory = true) {
            events {
                t["A", 0][CellWidth] = 25
            }
        }
    ...
    fun `listener order difference propagation`() {
        ...
        on(t["A"]) {
            ...
            events {
                ...
                assertEquals(t["A", 0][CellWidth], source[0][CellWidth])
    ...
    fun `old and new table is a clone of source table`() {
        ...
        on(t["A"]) {
            events {
                oldView["A", 0][CellWidth] = source[0].derived.cellWidth + 200
                newView["A", 0][CellWidth] = source[0].derived.cellWidth + 300

                assertEquals(source[0].derived.cellWidth + 200, oldView["A", 0][CellWidth].width)
                assertEquals(source[0].derived.cellWidth + 300, newView["A", 0][CellWidth].width)
    ...
    fun `type filtering for subscriptions`() {
        ...
        on<CellWidth<*, *>>(tv1["A"]) events {
            eventCount2 += count()
        }

        on<CellClasses<*>>(tv1["A"]) events {
            eventCount3 += count()
        }

        on<CellTopics<*>>(tv1["A"]) events {
            eventCount4 += count()
        }

        on<Resources>(tv1["A"]) events {
            eventCount5 += count()
        }

        on<SourceTable>(tv1["A"]) events {
            eventCount6 += count()
        }

        on<ColumnTransformer<*>>(tv1["A"]) events {
            eventCount7 += count()
        }
    ...
    fun `type filtering for chained subscriptions`() {
        ...
        on<CellWidth<*,*>>(tv1["A"], name = "Listener 2") events {
            eventCount2 += count()
            forEach {
