...
fun main() {
    ...
    val table = Table["DoughnutChart"]
    val tableView = TableView[table]

    val url = show(tableView, ref = "doughnut-chart", config = compactViewConfig(title = "Doughnut chart"))
    println(url)
    ...
    tableView["A", 0] = doughnut(
        null,
        listOf("X 1", "X 2", "X 3"),
        "Series A" to listOf(2.0, 1.5, 1.0)
    )

    tableView["B", 0] = doughnut(
        null,
        listOf("X 1", "X 2", "X 3"),
