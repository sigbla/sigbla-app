...
fun main() {
    ...
    val table = Table["PolarAreaChart"]
    val tableView = TableView[table]

    val url = show(tableView, ref = "polar-area-chart", config = compactViewConfig(title = "Polar area chart"))
    println(url)
    ...
    tableView["A", 0] = polarArea(
        null,
        listOf("X 1", "X 2", "X 3"),
        "Series A" to listOf(2.0, 1.5, 1.0)
    )

    tableView["B", 0] = polarArea(
        null,
        listOf("X 1", "X 2", "X 3"),
