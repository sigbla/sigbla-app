...
class TableRowCopyMove {
    ...
    fun `move rows within same table`() {
        ...
        Assert.assertEquals(listOf(listOf("A"), listOf("B")), headerOf(t).toList().map { it.header })
        Assert.assertEquals(listOf("A1", "A2", "A0", "A3"), valueOf<Any>(t["A"]).toList())
        Assert.assertEquals(listOf("B1", "B2", "B0", "B3"), valueOf<Any>(t["B"]).toList())
        // TODO indexOf test
        ...
        Assert.assertEquals(listOf(listOf("A"), listOf("B")), headerOf(t).toList().map { it.header })
        Assert.assertEquals(listOf("A2", "A0", "A1", "A3"), valueOf<Any>(t["A"]).toList())
        Assert.assertEquals(listOf("B2", "B0", "B1", "B3"), valueOf<Any>(t["B"]).toList())
        ...
        Assert.assertEquals(listOf(listOf("A"), listOf("B")), headerOf(t).toList().map { it.header })
        Assert.assertEquals(listOf("A1", "A0", "A2", "A3"), valueOf<Any>(t["A"]).toList())
        Assert.assertEquals(listOf("B1", "B0", "B2", "B3"), valueOf<Any>(t["B"]).toList())
